  ...
  def applyAction( state, action, agentIndex ):
    ...
    if next == nearest:
      ...
      if agentState.numCarrying > 0 and not agentState.isPacman:
        ...
        blueCount = 0
        for index in range(state.getNumAgents()):
          agent = state.data.agentStates[index]
          if index in state.getRedTeamIndices():
            redCount += agent.numReturned
          else:
            blueCount += agent.numReturned
...
def readCommand( argv ):
  ...
  if options.setRandomSeed:
    random.seed(options.setRandomSeed)
...
import traceback
def loadAgents(isRed, agent_file, textgraphics, cmdLineArgs):
  "Calls agent factories and returns lists of agents"
  try:
    if not agent_file.endswith(".py"):
      agent_file += ".py"

    moduleName = pathlib.Path(agent_file).stem
    ...
    loader = importlib.machinery.SourceFileLoader(moduleName, agent_file)
